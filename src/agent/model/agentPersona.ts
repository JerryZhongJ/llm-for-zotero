...
export const AGENT_PERSONA_INSTRUCTIONS: string[] = [
  ...
  [
    ...
    "Use paper_read({sections:[...]}) for section text (combine ['Abstract','Introduction','Conclusion'] for a broad picture), pages:[...] when the location is known, labels:[...] with images:true for extracted figure crops, pages with images:true for rendered page inspection, readFullReason only for an explicit exhaustive-read request, and paper_query({query}) for a specific missing claim. A passage read never satisfies an explicit full-read request.",
    "If there is no PDF attachment and the user needs more than local metadata or abstract evidence, one targeted external lookup is allowed and must be labeled separately.",
    "Use library_search for catalog discovery, library_read for structured item state, library_retrieve for evidence search and synthesis across a collection or library, paper_query for finding passages in known papers by content, and paper_read for close reading known papers.",

  ...
  return {
    ...
    guidance: {
      ...
      instruction:
        ...
        "When the note discusses a specific figure, first use `paper_read({ labels:['Figure N'], images:true })` and embed the extracted PDF crop path: `![Figure N](file:///{path})` — auto-imported as a Zotero attachment. " +
        "Treat paper_read figure extraction as the authority for figure crop cache reuse/regeneration; use returned crop paths as-is and do not inspect or validate `figure_crops` metadata before writing. " +
        "When the note discusses a table, use `paper_query({ query:'<table label and surrounding discussion>' })` for the table text and surrounding discussion instead of the figure-crop extractor. " +
        "If paper_read figure extraction returns no_figures, mineru_required, error, zero figures, or no image artifact, switch to text-only mode when the user asked for a note: do not include figure images, rendered PDF page screenshots, MinerU source images, or extracted-image placeholders; explicitly state that figure extraction failed or no extracted crops are available, and that explanations are based on captions, figure legends, and surrounding paper text. " +

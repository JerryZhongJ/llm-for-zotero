...
function decorateMcpToolDescription(
  ...
  const scopeGuidance =
    "Zotero MCP scope: omit libraryID, activeItemId, and activeContextItemId to use the current Codex Zotero chat scope. Use library_search with explicit entity and mode, for example library_search({ entity:'items', mode:'search', text:'...' }) or library_search({ entity:'collections', mode:'list', view:'tree' }), to discover Zotero items. Use library_retrieve for broad folder/library evidence search across a scoped resource pool: intent:'enumerate' for comprehensive quality-first local evidence search including which/all/how-many/list questions, intent:'summarize' for taxonomy/theme/commonality/comparison synthesis with body-evidence coverage in bounded selected pools, and intent:'verify' for exact presence/absence. Use library_read for structured item state, paper_read for close reading one known paper by structured coordinate (no arguments for an overview, sections:['...'] for whole sections, pages:[...] for exact pages, labels:[...] with images:true for precise extracted PDF figures, pages with images:true for rendered PDF pages/layout, readFullReason only for explicit exhaustive full-text requests with a coverage receipt), and paper_query({query}) for relevance-ranked textual evidence. Use literature_search for scholarly online search: it returns scholarly results (or canonical metadata patches with mode:'metadata') for source-cited answers. No general web-search MCP tool is available. For counting questions, prefer library_search totalCount/returnedCount/limited metadata or library_retrieve intent:'enumerate' coverage instead of hand-counting listed results.";
  const writeGuidance =
...
async function handleToolsCall(
  ...
  try {
    ...
    const prepared = await deps.toolRegistry.prepareExecution(
      ...
      {
        // The MCP surface is driven by an external agent bridge and carries
        // its own forced-confirmation policy, so the in-plugin intent
        // classifier requirement for model-originated writes does not apply.
        callerKind: "api",
        forceConfirmation:
